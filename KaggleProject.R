...
newkaggle_nba_2009_2011$homewin <-  with(newkaggle_nba_2009_2011, ifelse(h_FinalPoints > r_FinalPoints, "YES", "NO"))
...
library(ggplot2)
library(ggthemes)
ggplot(nbabet, aes(result))+ geom_bar(aes(fill=home_win)) + theme_excel()
ggplot(nbabet, aes(result))+ geom_bar(aes(fill=favdog)) + theme_pander()

ggplot(nbabet, aes(home_win)) + geom_bar(aes(fill=result)) + theme_hc()
ggplot(nbabet, aes(home_win)) + geom_bar(aes(fill=favdog)) + theme_solarized()


ggplot(nbabet, aes(favdog))+ geom_bar(aes(fill=result)) + theme_foundation()
ggplot(nbabet, aes(favdog))+ geom_bar(aes(fill=home_win)) + theme_stata()


#Actual count
count(nbabet, result)
count(nbabet, home_win)
count(nbabet, favdog)
count(nbabet, homewin)

#Plot density plot of pro_line and Final Line
ggplot(nbabet,aes(pro_line))+geom_density()+theme_excel_new()
ggplot(nbabet,aes(final_line))+geom_density()+ theme_gdocs()
...
#Combined plot #Learn how to add colour
library(GGally)
ggpairs(nbabet)
str(ggpairs(nbabet))
